const { ipcRenderer } = require('electron'); // EKLENEN KRİTİK SATIR
...
// KANAL İSMİ 'media-update' OLARAK DÜZELTİLDİ
ipcRenderer.on('media-update', (event, dataRaw) => {
...
// window.electronAPI YERİNE ipcRenderer KULLANILDI
document.getElementById('progress-bg').onclick = (e) => {
    ...
    if (parts.length >= 2) {
        ...
        ipcRenderer.send('send-command', `seek:${totalSec * percent}:${percent}`);
    }
};

document.getElementById('play-btn').onclick = () => ipcRenderer.send('send-command', 'playpause');
document.getElementById('next-btn').onclick = () => ipcRenderer.send('send-command', 'next');
document.getElementById('prev-btn').onclick = () => ipcRenderer.send('send-command', 'prev');
document.getElementById('vol-slider').oninput = (e) => ipcRenderer.send('send-command', `volume:${e.target.value}`);

// Opaklık için main.js'de ayar gerekecek ama şimdilik komutu gönderiyoruz
document.getElementById('op-slider').oninput = (e) => ipcRenderer.send('set-opacity', e.target.value); 
